...
echo
vpcTag="default-vpc"
subnetTag="default-subnet-a"
serverTag="cafeserver"
sgTag="default-sg"
type="t2.micro"
...
while [[ "$vpc" == "" ]] ; do
  for i in $(aws ec2 describe-regions | grep RegionName | cut -d '"' -f4) ; do
    region=$i;
    vpc=$(aws ec2 describe-vpcs --region $i --filters "Name=tag:Name,Values=$vpcTag" --profile $profile | grep VpcId | cut -d '"' -f4 | sed -n 1p );
    if [[ "$vpc" != "" ]]; then
      break;
    fi
  done
...
--filters "Name=tag:Name,Values=$subnetTag" \
--region $region \
--profile $profile \
...
--region $region \
--profile $profile \
--filters "Name=tag:Name,Values=$serverTag" "Name=instance-state-name,Values=running" \
...
#check for existing SG security Group
...
--region $region \
--subnet-id $subnetId \
--security-group-ids $existingMpSg \
--key-name $key \
--tag 'ResourceType=instance,Tags=[{Key=Name,Value='$serverTag'}]' \
...
echo
echo "The $serverTag is now running ..."
echo
echo "SSH to $serverTag (with .pem or .ppk added to the end of the keypair name):"
...
echo "step 4: ssh to the $serverTag using $key"
